// app/api/accept-single/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!bookingId || !colony || !assignedDate) {
      return NextResponse.json({ error: "bookingId, assignedDate va colony cookie talab qilinadi" }, { status: 400 });
    }
    ...
    const startDateZoned = toZonedTime(parseISO(assignedDate), timeZone);
    const createdDateZoned = toZonedTime(parseISO(booking.created_at), timeZone);
    const minDate = addDays(createdDateZoned, 10);

    if (startDateZoned < minDate) {
      return NextResponse.json({ error: "Дата должна быть не ранее 10 дней после создания заявки" }, { status: 400 });
    }
    ...
    // Получаем санитарные дни вокруг выбранной даты
    const queryStart = addDays(startDateZoned, -1);
    const queryEnd = addDays(startDateZoned, duration);
    const [sanitaryDays] = await pool.query<RowDataPacket[]>(
      ...
      [
        colony,
        formatInTimeZone(queryStart, timeZone, 'yyyy-MM-dd'),
        formatInTimeZone(queryEnd, timeZone, 'yyyy-MM-dd'),
      ]
    );
    ...
    for (let d = 0; d < duration; d++) {
      const day = addDays(startDateZoned, d);
    ...
    if (!isValidDate) {
      return NextResponse.json({ error: "Дата конфликтует с санитарным днем, выберите другую дату" }, { status: 400 });
    }
    ...
    if (!found || assignedRoomId === null) {
      return NextResponse.json({ error: "Все комнаты заняты, выберите другую дату" }, { status: 400 });
    }

    // Обновление заявки
    const startStr = formatInTimeZone(startDateZoned, timeZone, 'yyyy-MM-dd 00:00:00');
    const endStr = formatInTimeZone(addDays(startDateZoned, duration - 1), timeZone, 'yyyy-MM-dd 23:59:59');
    ...
    await pool.query(
      `UPDATE bookings 
       SET status = 'approved', 
           ...
           next_available_date = ? 
       WHERE id = ? AND colony = ?`,
      [startStr, endStr, assignedRoomId, booking.visit_type, nextAvailableStr, booking.id, colony]
    );
...
⌚ Kelish sanasi: ${formatInTimeZone(startDateZoned, timeZone, 'dd.MM.yyyy')}
...
`;
    ...
    const visitTypeTextRu = booking.visit_type === "short" ? "1-дневный" : booking.visit_type === "long" ? "2-дневный" : "3-дневный";
    const visitTypeTextUzl = booking.visit_type === "short" ? "1-kunlik" : booking.visit_type === "long" ? "2-kunlik" : "3-kunlik";
    const visitTypeTextUz = booking.visit_type === "short" ? "1-кунлик" : booking.visit_type === "long" ? "2-кунлик" : "3-кунлик";
...
⌚ Дата прибытия: ${formatInTimeZone(startDateZoned, timeZone, 'dd.MM.yyyy')}
⏲️ Тип: ${visitTypeTextRu}
...
⌚ Келиш санаси: ${formatInTimeZone(startDateZoned, timeZone, 'dd.MM.yyyy')}
⏲️ Тур: ${visitTypeTextUz}
